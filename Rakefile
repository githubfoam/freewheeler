...
namespace :upload do
  ...
  token = if ENV["VAGRANT_CLOUD_TOKEN_ALL"] || ENV["ATLAS_TOKEN"]
            ENV["VAGRANT_CLOUD_TOKEN_ALL"] || ENV["ATLAS_TOKEN"]
  ...
  @config["box"].each do |box|
    @config["provider"].each do |provider|
      ...
      task file do
        ...
        raise "Token is not defined either in config.yml, or environment variable VAGRANT_CLOUD_TOKEN_ALL" unless token
